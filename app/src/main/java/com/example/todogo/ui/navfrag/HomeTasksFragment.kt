...
import com.example.todogo.databinding.FragmentHomeTasksBinding
...
class HomeTasksFragment : Fragment() {
    ...
    private fun loadTasks() {
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            val tasks = taskRepositoryImp.getTasksByCategory("Home", FirebaseAuth.getInstance().currentUser?.uid.toString())
            withContext(Dispatchers.Main) {
                if (tasks.isNotEmpty()) {
    ...
    private fun setupRecyclerView() {
        binding.rvNoteList.apply {
            ...
            adapter = this@HomeTasksFragment.adapter
        }
    }
