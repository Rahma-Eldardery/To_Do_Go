...
import com.example.todogo.databinding.FragmentWorkTasksBinding
...
class WorkTasksFragment : Fragment() {
    ...
    private fun loadTasks() {
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            val tasks = taskRepositoryImp.getTasksByCategory("Work",  FirebaseAuth.getInstance().currentUser?.uid.toString())
            withContext(Dispatchers.Main) {
                if (tasks.isNotEmpty()) {
    ...
    private fun setupRecyclerView() {
        binding.rvNoteList.apply {
            ...
            adapter = this@WorkTasksFragment.adapter
        }
    }
